...
- Current authority: this repository after baseline anchoring.
...
The first deployment is the existing Hermes VM with 8 vCPU, 16 GiB RAM and no GPU. The Proxmox host has more total CPU/RAM, but those resources are not automatically available inside the VM.
...
Validation passes, baseline commit is anchored, and Hermes completes read-only takeover.

## Blockers and risks

No technical blocker for takeover. Live measurements must be refreshed read-only. UI-00 must be accepted before backend. Engine selection remains a measured spike.
